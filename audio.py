...
class speechEmotionRecognition:
    ...
    def __init__(self, subdir_model=None):
        ...
        if subdir_model is not None:
            ...
            self._model = self.build_model()
            self._model.load_weights('models/MODEL_CNN_LSTM.hdf5')  
    ...
    def prediction_to_csv(self, predictions, filename, mode='w'):
        ...
        with open(filename, mode) as f:
            ...
            for emotion in predictions:
                f.write(str(emotion)+'\n')
            f.close()
    def combine_emotion_predictions(self,predictions):
        """
        Combines emotion predictions from multiple timestamps and returns the final emotion.

        Args:
        predictions (list of list of float): List of emotion predictions from each timestamp.

        Returns:
        list: The aggregated scores for each emotion.
        int: The index of the final emotion with the highest aggregated score.
        """
        # Convert the predictions list into a numpy array
        predictions_array = np.array(predictions)

        # Sum the scores for each emotion across all timestamps
        aggregated_scores = np.sum(predictions_array, axis=0)

        # Determine the final emotion with the highest aggregated score
        final_emotion_index = np.argmax(aggregated_scores)

        return self._emotion.get(final_emotion_index)
if __name__ == '__main__':
    # Path to your model file and audio file
    model_path = 'models/MODEL_CNN_LSTM.hdf5'
    audio_file = 'audio_data/harvard.wav'
    # Initialize the speech emotion recognition with the pre-trained model
    ser = speechEmotionRecognition(subdir_model=model_path)

    # Predict emotion from the audio file
    predictions, timestamps = ser.predict_emotion_from_file(audio_file)

    # Print the predictions and timestamps
    for emotion, timestamp in zip(predictions, timestamps):
        print(f"Timestamp: {timestamp} seconds - Emotion: {emotion}")
    print(ser.combine_emotion_predictions(predictions))